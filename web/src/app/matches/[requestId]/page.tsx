...
export default function MatchesPage() {
  ...
  return (
    <div className="space-y-8 animate-fadeUp">
      <div className="space-y-2">
        <h1 className="text-3xl font-bold text-leeds-blue tracking-tight">
          Recommended Helpers
        </h1>
        <p className="text-leeds-blue-dark/70">
          We found these experts for your request{" "}
          <span className="font-semibold text-leeds-teal">#{requestId}</span>.
        </p>
      </div>
      ...
      {loading ? (
        <div className="grid md:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-48 rounded-2xl bg-white border border-leeds-border animate-pulse"
            />
          ))}
        </div>
      ) : errorMessage ? (
        ...
        <div className="text-center py-12 bg-white rounded-2xl border border-leeds-border">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Try updating your request with more popular tags.
          </p>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {matchList.map((match) => {
            const isRequesting = requestingIds.has(match.id);
            const scorePercent = Math.round(match.score * 100);
            const state = (match as any).state as MatchState | undefined; // MatchCard now includes it
